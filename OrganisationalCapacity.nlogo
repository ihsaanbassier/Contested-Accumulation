...
globals
[
  id1-share             ;; share of gian component of breed white
  ...
  avg-id1 avg-id2 avg-share sims
]
...
to setup
  clear-ticks clear-turtles clear-patches clear-all-plots
  set id1-share 0 set component-size 0 set giant-component-size 0
  set giant-start-node 0
...
to make-turtles
  create-id1s num-nodes / 2 [
    set shape "circle"
    set size 3
    set rate initial-id1
  ]
  create-id2s num-nodes [
    set shape "x"
    set size 3
    set rate initial-id2
  ]
...
to go
  ...
  add-edge
  if giant-component-size > max-size and random-float 1 < 0.2 [ drop-edge ]
  find-all-components
  color-giant-component
...
to update-rates
  set id1-share count id1s with [ color = red ] / giant-component-size
  ask id1s [
    ifelse id1-share > 0.5
    [ set rate rate + ( id1-share - 0.5 ) * (1 - rate) * ( giant-component-size / count turtles ) ]
    [ set rate rate + ( id1-share - 0.5 ) * (rate + 1) * ( giant-component-size / count turtles ) ]
  ]

  ask id2s [
    ifelse id1-share < 0.5
    [ set rate rate + ( 0.5 - id1-share ) * (1 - rate) * ( giant-component-size / count turtles ) ]
    [ set rate rate + ( 0.5 - id1-share ) * (rate + 1) * ( giant-component-size / count turtles ) ]
  ]
  ;; find proportion with color red that is breed white/black
  ;; (proportion - 0.5) * (1 - current rate)
  ;; negative if smaller. 50% => no change.
  ;; normalise by proportion in giant * proportion links in giant
end
...
to add-edge
  ...
  ask node1 [
    ...
    ;; ifelse giant-component-size >= max-size and ([color] of node1 = red and [color] of node2 != red) or ([color] of node1 != red and [color] of node2 = red)
    ;;  [ add-edge]
    ...
    [ if rate + [ rate ] of node2 > random-float 2 [ create-link-with node2 ] ]

  ]
end

to drop-edge
  ask one-of links with [color = red ] [ die ]
end
...
to simulate
  ...
  while [sims < 30 ] [
    ...
    ifelse sims = 1 [
      ifelse one-of [rate] of id1s > 0
      [ set avg-id1 1 set avg-id2 0 ]
      [ set avg-id1 0 set avg-id2 1 ]
      set avg-share id1-share
    ]
    [
      ifelse one-of [rate] of id1s > 0
      [ set avg-id1 ( avg-id1 * sims + 1 ) / ( sims + 1)
        set avg-id2 ( avg-id2 * sims + 0 ) / ( sims + 1) ]
      [ set avg-id1 ( avg-id1 * sims + 0 ) / ( sims + 1)
        set avg-id2 ( avg-id2 * sims + 1 ) / ( sims + 1) ]
      set avg-share ( avg-share * sims + id1-share ) / ( sims + 1)
    ]
  ]
  print " Average id1 dominance: " print avg-id1
  print " Average id2 dominance: " print avg-id2
  print " Average id1 share: " print avg-share
...
"" ""
PENS
"id1 rate" 1.0 0 -14835848 true "" "plot one-of [ rate ] of id1s"
"id2 rate" 1.0 0 -16777216 true "" "plot one-of [ rate ] of id2s"
"id1-share" 1.0 0 -2674135 true "" "plot id1-share"
